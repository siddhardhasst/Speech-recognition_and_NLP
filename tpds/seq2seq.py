...
lines_ = [lines[0]]
while i < len(lines):
	if lines[i].split('\t')[0] != lines_[len(lines_)-1].split('\t')[0]:
...
for line in lines[: min(num_samples, len(lines) - 1)]:
	...
	input_text.translate(None, string.punctuation)
	input_text.translate(None, '.')
	target_text.translate(None, string.punctuation)
	target_text.translate(None, '.')
...
input_characters = set (ip_all.split())
